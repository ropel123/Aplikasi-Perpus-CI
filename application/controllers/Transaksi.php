...
class Transaksi extends CI_Controller
{
	...
	public function index()
	{
		...
		if ($this->session->userdata('level') == 'Anggota') {
			$this->data['pinjam'] = $this->db->query(
				...
				`status`, `tgl_pinjam`, `lama_pinjam`, `tgl_balik`, `tgl_kembali`, `buku_id`
	...
	public function pinjammember()
	{
		...
		$this->data['buku'] =  $this->db->query("SELECT * FROM tbl_buku ORDER BY id_buku DESC");
		$this->session->set_userdata('pp', $this->uri->segment(3));
		$this->data['title_web'] = 'Tambah Pinjam Buku ';

		$this->load->view('header_view', $this->data);
		$this->load->view('sidebar_view', $this->data);
		$this->load->view('pinjam/tambahag_view', $this->data, 'refresh');
		$this->load->view('footer_view', $this->data);
	}
	public function download()
	{
		$pol = $this->uri->segment('3');
		$result = $this->db->query("SELECT DISTINCT  `lampiran` FROM tbl_buku WHERE buku_id='$pol'")->result_array();
		$dir = './assets_style/image/buku/';
	...
	public function prosespinjam()
	{
		...
		if (!empty($post['tambah'])) {
			...
			if ($total_array != 0) {
				...
				$this->db->update_batch('tbl_buku', $pol, 'buku_id');
