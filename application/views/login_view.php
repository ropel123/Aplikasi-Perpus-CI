...
<body class="hold-transition login-page" style="overflow-y: hidden;background:url(
  ...
  <div class="login-box">
    ...
    <?php if ($this->session->userdata('error')) { ?>
      <div class="error" data-error="<?= $this->session->userdata('error'); ?>" data-gambar="<?= base_url('login') ?>"></div> <?php
    ...
    <div class="login-box-body" style="border:2px solid #226bbf;">
      ...
      <form action="<?= base_url('login/auth'); ?>" method="POST">
        ...
        <div class="row">
          ...
          <div class="col-xs-4">
            <a href="<?= base_url('login/adduser') ?>" class="btn btn-primary btn-block btn-flat">Daftar Akun</a>
            <div id="loadingcuy"></div>
          </div>
          <div class="col-xs-4">
            <button type="submi id=" loding" class="btn btn-primary btn-block btn-flat">Login</button>
            <div id="loadingcuy"></div>
          </div>
          <!-- /.col -->
        </div>
    ...
    <footer>
      <div class="login-box-body text-center bg-blue">
        <a style="color: yellow;"> Copyright &copy; Sistem Perpustakaan Kelompok 3 - <?php echo date("Y"); ?>
      </div>
    </footer>
  </div>
  <!-- /.login-box -->
  ...
  <script>
    const flashdata = $('.error').data('error');
    const url = $('.error').data('gambar');
    if (flashdata) {
      Swal.fire({
        position: 'center',
        icon: 'error',
        ...
        backdrop: `
    rgba(0,0,123,0.4)
    url("./assets_style/image/buku/cat1.gif")
    left top
    no-repeat
  `
      })
