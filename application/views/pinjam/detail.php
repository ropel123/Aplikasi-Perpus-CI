						...
						<div class="row">
							<div class="col-sm-5">
								<table class="table table-striped">
									...
									<tr>
										<td>Sisa Durasi Peminjaman</td>
										<td>:</td>
										<td>
											<p id="coldon" data-akhr="<?= $pinjam->tgl_balik; ?>"></p>
										</td>
									</tr>
								</table>
							</div>
...
<script>
	function countdown(t) {
		...
		var x = setInterval(function() {
			...
			m = ("0" + m).slice(-2);
			s = ("0" + s).slice(-2);
			var cd = '<strong>' + d + " Hari, " + h + " Jam " +
				'</strong>';
			$('p#coldon').html(cd);
			setTimeout(function() {
				location.reload()
				$('p#coldon').html('Waktu Sudah habs');
			}, dis);
		}, 1000);
	}
</script>